...
build_bootstrap_jdk12() {
	cd "$DOWNLOAD_DIR"
	curl -O -L https://github.com/AdoptOpenJDK/openjdk12-binaries/releases/download/jdk-12%2B33/OpenJDK12U-jdk_x64_mac_hotspot_12_33.tar.gz
	cd "$TOOL_DIR"
	tar -xvf "$DOWNLOAD_DIR/OpenJDK12U-jdk_x64_mac_hotspot_12_33.tar.gz"
}
...
buildtools() {
	...
	for tool in autoconf mercurial bootstrap_jdk12 ; do 
		echo "building $tool"
		build_$tool
	done
}
...
export PATH=$OLDPATH
export JAVA_HOME=$TOOL_DIR/jdk-12+33/Contents/Home
...
downloadjdksrc() {
	cd $BUILD_DIR
	hg clone http://hg.openjdk.java.net/jdk/jdk $JDK_DIR
	cd $JDK_DIR
	hg revert .
}
